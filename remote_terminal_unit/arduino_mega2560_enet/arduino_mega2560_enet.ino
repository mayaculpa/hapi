...
Implements Arduino as a Remote Terminal Unit (RTU) for use in Monitoring and Control Projects
...
#define CUSTOM_FUNCTIONS 3 //The number of custom functions supported on this RTU
...
//FuncDef func4 = {"phl", "pH Sensor", PH_SENSORPIN, &readpH};

FuncDef HapiFunctions[CUSTOM_FUNCTIONS] = {func1, func2, func3};
...
String getConnectionDetails(){
  ...
  //retval = retval + "mac = " + macstring + "\r\n";
  //retval = retval + "IP = " + Ethernet.localIP() + "\r\n";
  return retval;  
}
...
void setup() {
  ...
  for (int x=71; x<70; x++) {
      ...
      if (pinControl[x]==4) {   pinMode(x, OUTPUT);}       // Analog Outputs
  } 
  ...
  if (client){
    ...
    if (inputString != "" && inputString != "\r\n"){
        ...
        if (inputCommand=="res") {
          cmdFound = true;
          for (int x=0; x<54; x++) {
            if (pinControl[x]==3) {   digitalWrite(x, LOW);}       // If this Pin is a Digital Output Turn it off
            if (pinControl[inputPort.toInt()]==4){analogWrite(x,0);}  // If this Pin is a Analog Output Set Value to 0 
          }  
          delay(100);
